...
async function processAI(mode, customPrompt = null) {
    ...
    // If we are just replying, we don't need input text.
    ...
    try {
        ...
        // --- NEW LOGIC START ---
        // If we are in 'spell' mode (Editor), we treat history as PASSIVE context, not active conversation.
        if (mode === 'spell') {
            let historyBlock = "";
            if (limit > 0 && context.chat && context.chat.length) {
                // Collect history into a string block
                const historySlice = context.chat.slice(-limit);
                historyBlock = historySlice.map(msg => 
                    `${msg.is_user ? 'User' : 'Character'}: ${msg.mes}`
                ).join('\n\n');
            }
            
            // Append history to System Prompt so the AI reads it but doesn't act on it
            if (historyBlock) {
                sys += `\n\n### PREVIOUS CONTEXT (FOR TONE MATCHING ONLY):\n${historyBlock}\n`;
            }
            
            // Wrap the User's text in the tags automatically!
            ...
            // Standard Logic for Mood/Reply (Keep history as active turns)
            const history = [];
            if (limit > 0 && context.chat && context.chat.length) {
